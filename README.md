<h1>Hey mate! Jolly good to see you <img src="https://i.imgur.com/LQEEC3e.png" width="56" height="40"></h1>

### A short, complimentary rundown of what you can expect to find here:  

https://github.com/x0reaxeax/x0reaxeax/assets/61374847/a0206544-a96b-4549-bf60-7afbb53a6b13
    

![eye24](eye24.ico) Website: [x0reaxe.ax](http://x0reaxe.ax)  
![yt24](yt24.ico) YouTube: [@x0reaxeax](https://youtube.com/@x0reaxeax)  


## Gaming stuff
- Grounded 2
    - [Grounded2Minimal - Grounded2 trainer](https://github.com/x0reaxeax/Grounded2Minimal)
    - [G2Dumper](https://github.com/x0reaxeax/G2Dumper)
- Grounded
    - [GroundedMinimal - Grounded trainer](https://github.com/x0reaxeax/GroundedMinimal)
- Need for Speed
    - Most Wanted (2005)
        - [MW2005-HShifter - H-Shifter support](https://github.com/x0reaxeax/MW2005-HShifter)
        - [MWCrashFix - 5iq Crash "Fix"](https://github.com/x0reaxeax/MWCrashFix)
    - Heat
        - [nfsheat-hshifter - H-Shifter support](https://github.com/x0reaxeax/nfsheat-hshifter)
- Mad Max
    - [MadMaxCrashFix - Startup Crash Fix](https://github.com/x0reaxeax/MadMaxCrashFix)
- Resident Evil 6
    - [RE6CrashFix - Startup Crash Fix](https://github.com/x0reaxeax/RE6CrashFix)
- GTA 5
    - [DriftGripV - Drift Grip (Enhanced + Legacy)](https://github.com/x0reaxeax/DriftGripV)
- Minecraft
    - [MalisisDoors - MalisisDoors fork with fixes](https://github.com/x0reaxeax/MalisisDoors)
    - [TimeGate - Forge 1.7.10 Dupe Helper](https://github.com/x0reaxeax/TimeGate)
    - [MeeCreeps - MeeCreeps fork with additional features](https://github.com/x0reaxeax/MeeCreeps)
- Palia
    - [PaliaMinimal2 - Force item placement / building restriction remover tool](https://github.com/x0reaxeax/PaliaMinimal2)
- Saleblazers
    - [SaleblazersFreeMove - Building restriction remover](https://github.com/x0reaxeax/SaleblazersFreeMove)
